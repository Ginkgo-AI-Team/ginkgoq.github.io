...
(function () {
  ...
  function mount() {
    ...
    const E0  = edge[0];
    const EL  = edge[edge.length - 1];
    const E1  = edge[1];
    const EL1 = edge[edge.length - 2];
    ...
    const lDir  = unit(E1.x - E0.x, E1.y - E0.y);
    const lArc  = Math.hypot(stemTop.x - E0.x, stemTop.y - E0.y);
    const lCp2  = { x: fmt(E0.x - lDir.x * lArc * 0.22), y: fmt(E0.y - lDir.y * lArc * 0.22) };
    const lCp1  = { x: fmt(stemTop.x + (E0.x - stemTop.x) * 0.28), y: fmt(stemTop.y + (E0.y - stemTop.y) * 0.22) };

    const rDir  = unit(EL.x - EL1.x, EL.y - EL1.y);
    const rArc  = Math.hypot(stemTop.x - EL.x, stemTop.y - EL.y);
    const rCp1  = { x: fmt(EL.x + rDir.x * rArc * 0.22), y: fmt(EL.y + rDir.y * rArc * 0.22) };
    const rCp2  = { x: fmt(stemTop.x + (EL.x - stemTop.x) * 0.28), y: fmt(stemTop.y + (EL.y - stemTop.y) * 0.22) };
